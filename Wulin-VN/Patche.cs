...
class Patches
{
    ...
    private static void TextMeshProUGUI_OnEnable_Postfix(TextMeshProUGUI __instance)
    {
        ...
        if (VyyyTranslationConfig.ReplaceFont && VyyyTranslationConfig.MainFontAsset != null && VyyyTranslationConfig.MainFontAsset.name != __instance.font.name)
        {
            ...
            VyyyTranslationConfig.CheckAndAddFontFallback(__instance.font.fallbackFontAssets, font);
            __instance.font = VyyyTranslationConfig.MainFontAsset;

        }

        string gameObjectPath = VyyyTranslationConfig.GetGameObjectPath(__instance.gameObject);
        foreach (Il2CppSystem.Collections.Generic.KeyValuePair<string, string> customConfigDict in VyyyTranslationConfig.CustomConfig)
        {
    ...
    private static void LocalizationComponent_Awake_Postfix(LocalizationComponent __instance)
    {
        ...
        if (VyyyTranslationConfig.Dump == true)
        {
            ...
            for (int i = 0; i < locDatas.Length; i++)
            {
                ...
                //jsonObject["original"] = locData.SChinese;
