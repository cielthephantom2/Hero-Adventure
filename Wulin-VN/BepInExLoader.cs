...
[BepInPlugin("com.Vyyy.Translation", "VyyyTranslation", "1.0.2")]
...
public class BepInExLoader : BasePlugin
{
    ...
    public override void Load()
    {
        ...
        VyyyTranslationConfig.LoadConfigFile();
        VyyyTranslationConfig.LoadBundleFile();
        VyyyTranslationConfig.LoadFontFile();
        VyyyTranslationConfig.CreateTMP("FontModded");
        if (VyyyTranslationConfig.MainFontAsset != null)
        {
            TMP_Settings s_Instance = TMP_Settings.s_Instance;
            TMP_FontAsset defaultFontAsset = s_Instance.m_defaultFontAsset;
            if (defaultFontAsset != null && defaultFontAsset.name != VyyyTranslationConfig.MainFontAsset.name)
            {
                s_Instance.m_defaultFontAsset = VyyyTranslationConfig.MainFontAsset;
                if (s_Instance.m_fallbackFontAssets == null)
                {
                    s_Instance.m_fallbackFontAssets = new List<TMP_FontAsset>();
                }
                VyyyTranslationConfig.CheckAndAddFontFallback(s_Instance.m_fallbackFontAssets, defaultFontAsset);
            }
        }
        BepInExLoader.translationDict = JSONFileNONLocDataToDictionary("LocData.json");
        Harmony harmony = new Harmony("com.Vyyy.Translation");
